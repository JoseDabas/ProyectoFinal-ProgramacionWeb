package edu.pucmm.eict.ormjpa.modelos;
...
@Entity
@NamedQueries({
        @NamedQuery(name = "Clase.findAllProfesorId", query = "select p from GrupoClase p where p.profesor.id = :id") })
public class GrupoClase implements Serializable {
    ...
    @ManyToOne()
    private Profesor profesor; // Relación mucho a uno con el profesor, un profesor puede tener muchas clases,
                               // pero una clase un profesor.
